...
let documents = JSON.parse(JSON.stringify(documentsData));
let pages = 1;
let pageNow = 1;
...
function CreateAndAdd(resData) {
  ...
  if (resData) {
    resData.map((el, ind) => {
      let name_menu_item = `<div class="drop_down_menu_item"><span>${el.name.trim()}</span></div>`;
      let step_menu_item = `<div class="drop_down_menu_item"><span>${el.development_step.trim()}</span></div>`;
      let state_menu_item = `<div class="drop_down_menu_item"><span>${el.status.trim()}</span></div>`;
      let category_menu_item = `<div class="drop_down_menu_item"><span>${el.category.trim()}</span></div>`;
      let document_menu_item = `<div class="drop_down_menu_item"><span>${documents[ind].documents.length}</span></div>`;
      let tk_menu_item = `<div class="drop_down_menu_item"><span>${el.tk}</span></div>`;
      let clsfn_menu_item = `<div class="drop_down_menu_item"><span>${el.classification.trim()}</span></div>`;
      el.name.trim() ? AddFilterMenuItem(name_menu_item, "name", el.name):""
      el.development_step.trim() ? AddFilterMenuItem(step_menu_item, "step", el.development_step):""
      el.status.trim() ? AddFilterMenuItem(state_menu_item, "state", el.status):""
      el.category.trim() ? AddFilterMenuItem(category_menu_item, "category", el.category):""
      documents[ind].documents.length>0? AddFilterMenuItem(document_menu_item, "docum", documents[ind].documents.length):""
      el.tk.trim() ? AddFilterMenuItem(tk_menu_item, "tk", el.tk):""
      el.classification.trim() ? AddFilterMenuItem(clsfn_menu_item, "clsfn", el.classification):""
        ...
        <div class="gost_line">
            ...
            <div class="gost_line_docum gost_line_item">
              <a class="document_amount_item" href="../pages/document.html?docum=${
                ...
                <span>${documents[ind].documents.length}</span>
              </a>
            </div>
  ...
  data.map((el, ind) => {
    if (
      ...
      CheckLines(
        ...
        documents[ind].documents.length
      ) &&
      CheckLines(
        "tk",
...
function ChooseFilterInputItem(byWhat) {
  if (byWhat) {
    ...
    if (filterMenu) {
      for (const i in filterMenu) {
        if (typeof filterMenu[i] == "object" && filterMenu[i]!=null ) {
          filterMenu[i].addEventListener("click", function () {
